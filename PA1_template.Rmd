...
Since we'll be using `group_by()` and related commands to figure this out we need to install and load the `dplyr` and 'magrittr` (the last one supports the pipe operator) packages:
...
Result: `r round(mean_total_steps, 0)` steps.
