...
So, on most days the number of steps still lies between 10.000 and 15.000. There's hardly any visible difference when compared to the histogram that was created using the original (non-imputed) dataset.
...
The mean total number of steps for the imputed dataset is `r mean_total_steps.imp$mean_total_steps`. This is only slightly more than the mean for the original data (`r mean_steps`). In relative terms the difference of `r mean_total_steps.imp$mean_total_steps - mean_steps` represents a (neglectable?) difference of only `r (mean_total_steps.imp$mean_total_steps - mean_steps) / mean_steps` percent.
